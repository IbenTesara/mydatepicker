...
export class MyDatePicker implements OnChanges, ControlValueAccessor {
	...
	resetMonthYearSelect(): void {
		this.selectMonth = this.opts.selectMonthOnly;
	...
	onYearCellClicked(cell: IMyCalendarYear): void {
		if (this.options.selectYearOnly) {
			var dateModel = this.getDateModel({
				day: 1,
				month: 1,
				year: cell.year
			});
		...
		if (this.options.selectMonthOnly) {
            ...
            this.nextYearDisabled = cell.year >= new Date().getFullYear();
	...
	generateYears(year: number): void {
		...
		for (let i = year; i <= 20 + year; i += 5) {
			...
			for (let j = i; j < i + 5; j++) {
				const disabled = j < this.opts.disableUntil.year || j > this.opts.disableSince.year;
        ...
        this.prevYearsDisabled = year < this.opts.disableUntil.year;
        this.nextYearsDisabled = year > this.opts.disableSince.year;
		this.cdr.markForCheck();
	}
			...
			for (let i = 1; i <= 12; i += 3) {
				...
				for (let j = i; j < i + 3; j++) {
					...
					row.push({
						...
						selected: j === this.selectedDate.month && this.visibleMonth.year === this.selectedDate.year,
						disabled: disabled
					});
				}
				this.months.push(row);
	...
	generateCalendar(m: number, y: number, notifyChange: boolean): void {
		...
		if (y !== new Date().getFullYear()) {
			this.months = this.months.map(monthRow =>
				monthRow.map(row => {
					return Object.assign({}, row, {
						disabled: false,
						currMonth: today.month === row.nbr && today.year === y,
						selected: this.selectedDate.month === row.nbr && y === this.selectedDate.year
					});
				})
			)
	...
	isCurrentWeek(data: {weekNbr: number; week: {dateObj: {year: number}}[]}) {
		const year = new Date().getFullYear();

		return data.weekNbr === this.currWeek && data.week.some(x => x.dateObj.year === year);
	}

	isSelectedWeek(data: {weekNbr: number; week: {dateObj: {year: number}}[]}) {
		return this.opts.markCurrentWeek && this.opts.selectWeekOnly && data.weekNbr === this.selectedDate.week && data.week.some(x => x.dateObj.year === this.selectedDate.year);
	}
}
